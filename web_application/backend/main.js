...
mongo.connect((error, client) => {
  ...
  exp.locals.sold_tickets = client.db(db_name).collection("sold_tickets");
...
// ... для запиту /get_sold_tickets
exp.get("/get_sold_tickets", (req, res) => {

  collection = req.app.locals.sold_tickets;
  collection.find({}).toArray((error, result) => {
...
exp.put(["/set_trains",
  ...
  switch (collection) {
    ...
    case 4: collection = req.app.locals.sold_tickets; break;
